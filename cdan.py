...
from models import ConditionalDomainAdversarial, DomainDiscriminator
...
def main(args: argparse.Namespace):
    ...
    if args.mode == "train":
        ...
        cdan = ConditionalDomainAdversarial(backbone, classifer, domainDiscriminator, n_class, args.trade_off)
        ...
        trainer.fit(cdan, train_dataloader, valid_dataloader)

        ## testing with the best model
        trainer.test(cdan, [test_src_dataloader, test_trg_dataloader], ckpt_path='best')
    elif args.mode == "analysis":
        if args.best_ckpt is None:
            raise ValueError("Please specify the best checkpoint path")
        
        cdan = ConditionalDomainAdversarial.load_from_checkpoint(
